...
import { AkashicRecordsBackend } from "../../src/AkashicRecordsBackend";
import { AkashicRecordsConfiguration } from "../../src/AkashicRecordsConfiguration";
import { AkashicRecordsGlobal } from "../../src/AkashicRecordsGlobal";
...
const main = async (): Promise<void> => {
  ...
  AkashicRecordsGlobal.testing = true;

  // BACKEND SERVER
  const backend: AkashicRecordsBackend = new AkashicRecordsBackend();
  await backend.open();
  ...
  try {
    await fs.promises.mkdir(
      `${AkashicRecordsConfiguration.ROOT}/docs/benchmarks`,
      {
        recursive: true,
      }
    );
  ...
  await fs.promises.writeFile(
    `${AkashicRecordsConfiguration.ROOT}/docs/benchmarks/${os
      .cpus()[0]
      .model.trim()
      .split("\\")
      .join("")
